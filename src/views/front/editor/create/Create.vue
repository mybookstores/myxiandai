  ...
  <div class="create">
    <div class="header" style="font-size: 16px;font-weight: bold;">
      标题<el-input
        ...
        style="max-width: 250px;margin-left: 25px;font-size: 14px;"
        @blur="updatePaper()"
      />
    </div>
    ...
    <div style="display: flex;align-items: center;">
      <span  style="font-size: 16px;font-weight: bold;margin-right: 15px;">问卷说明</span>
      <el-input
        type="textarea"
        :autosize="{ minRows: 2, maxRows: 4 }"
        placeholder="请输入问卷说明"
        v-model="paperInfo.description"
        style="width: 800px; padding-right: 100px;font-size: 14px;"
        @blur="updatePaper()"
      >
      </el-input>
    </div>
    ...
    <el-form>
      <el-form-item
        ...
        :key="index"
      >
      <div v-if="ques.type === 1" style="margin-bottom: 15px;font-weight: bold;font-size: 16px;">单选题</div>
      <div v-if="ques.type === 2" style="margin-bottom: 15px;font-weight: bold;font-size: 16px;">多选题</div>
      <div v-if="ques.type === 3" style="margin-bottom: 15px;font-weight: bold;font-size: 16px;">简答题</div>
        <el-input
          type="textarea"
          placeholder="请输入问题"
          v-model="ques.title"
          style="width:766px; border: 0px solid; display: block;font-size: 14px;margin-bottom:20px;"
        >
        </el-input>
        <div v-if="ques.type === 1">
          <!-- <el-tag type="info" style="margin-top: 10px">单选题</el-tag> -->
          <el-button
            style="display: block; margin-top: 10px;background-color: #d11579;border: #d11579;margin-bottom:20px;"
            type="primary"
            size="small"
            icon="el-icon-plus"
          ...
          >
            ...
            <el-button
              ...
              circle
              size="mini"
              style="margin-left: 5px;background-color:#f7b100;border: #f7b100;width:25px;height: 25px;"
        ...
        <div v-else-if="ques.type === 2">
          ...
          <el-button
            style="display: block; margin-top: 10px;background-color: #d11579;border: #d11579;margin-bottom:10px;"
            type="primary"
            size="small"
            icon="el-icon-plus"
            @click="addOption(ques)"
        ...
        <div style="display: flex; justify-content: flex-end;">
          <el-button @click="delQues(index)"  size="small" type="primary"  >删除</el-button>
          <el-button @click="updQues(index)"  size="small" type="success"  style="background-color: #725ae9;border: #725ae9;">保存</el-button>
    ...
    <div style="padding-bottom: 20px">
      <el-button v-if="Listempty" type="primary"  size="small" @click="submitpaper()"
        >发放问卷 !</el-button
      >
      <el-divider v-if="Listempty" direction="vertical" />
      <el-dropdown @command="addQues">
        <el-button type="primary" size="small" style="background-color: #725ae9;border: #725ae9;" 
  ...
  methods: {
    ...
    submitpaper: async function() {
      ...
      for (let ques of this.questionList) {
        if (ques.title === '') {
          this.$message.warning(`问题 ${i} 题目描述不可为空`)
          return
        }
        if (ques.type <= 2) {
          if (ques.options.length < 2) {
          ...
          for (let option of ques.options) {
            if (option.content.length === 0) {
              this.$message.warning(`问题 ${i} 选项不可为空`)
              return
            }
          }
        }
