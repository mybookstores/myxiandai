      ...
      <div v-for="(paper, index) in allPapers" :key="paper.id" style="display:block">
        <el-card class="paper_card" style="margin-bottom:30px">
          <div slot="header" class="clearfix">
            ...
            <span style="margin-left: 5px;font-weight: bold;">标题： {{ paper.title }}</span>
            <label v-if="paper.status === 'INIT'" style="float: right;font-size:15px;"
              >问卷状态：编辑中</label
            >
            <label v-else-if="paper.status === 'START'" style="float: right;font-size:15px;">
              问卷状态：已发放
            </label>
            <label v-else-if="paper.status === 'STOP'" style="float: right;font-size:15px;">
              问卷状态：已回收
            </label>
          </div>
          <el-button
